...
ADD COLUMN Oficio VARCHAR(30),
ADD COLUMN Comision FLOAT;

-- 1.Empleados entre fechas y por departamento
DELIMITER //
CREATE PROCEDURE EmpleadosPorFechaYDepartamento(
    IN p_FechaInicio DATE,
    IN p_FechaFin DATE,
    IN p_IdDepartamento CHAR(5)
)
BEGIN
    SELECT *
    FROM Empleados
    WHERE FechaContrato BETWEEN p_FechaInicio AND p_FechaFin
...
CREATE PROCEDURE InsertarEmpleado(
    ...
    IN p_Direccion VARCHAR(40),
    IN p_Correo VARCHAR(40),
    IN p_NumSS VARCHAR(30),
    IN p_Telefono VARCHAR(15),
    IN p_IdDepartamento CHAR(5),
    IN p_Oficio VARCHAR(30),
    IN p_Comision FLOAT
)
BEGIN
    INSERT INTO Empleados
    VALUES(p_IdEmpleado, p_Nombre, p_FechaContrato, p_Direccion,
           p_Correo, p_NumSS, p_Telefono, p_IdDepartamento,
           p_Apellido, p_Oficio, p_Comision);
END //

-- 3.Nombre departamento, número depto y total personas
CREATE PROCEDURE InfoDepartamentoPorNumero(
    IN p_IdDepartamento CHAR(5)
)
BEGIN
    SELECT d.NombreD,
           d.IdDepartamento,
           COUNT(e.IdEmpleado) AS TotalPersonas
    FROM Departamentos d
    LEFT JOIN Empleados e 
        ON d.IdDepartamento = e.IdDepartamento
    WHERE d.IdDepartamento = p_IdDepartamento
    GROUP BY d.NombreD, d.IdDepartamento;
END //

-- 4.Igual que anterior pero por nombre de departamento
CREATE PROCEDURE InfoDepartamentoPorNombre(
    IN p_NombreDepartamento VARCHAR(20)
)
BEGIN
    SELECT d.NombreD,
           d.IdDepartamento,
    ...
    LEFT JOIN Empleados e 
        ON d.IdDepartamento = e.IdDepartamento
    WHERE d.NombreD = p_NombreDepartamento
    GROUP BY d.NombreD, d.IdDepartamento;
END //

-- 5.Salario, oficio y comisión por apellido
CREATE PROCEDURE DatosEmpleadoPorApellido(
    IN p_Apellido VARCHAR(20)
)
BEGIN
    SELECT d.Sueldo,
    ...
    JOIN Departamentos d 
        ON e.IdDepartamento = d.IdDepartamento
    WHERE e.Apellido = p_Apellido;
END //

-- 6.Igual que anterior
CREATE PROCEDURE DatosEmpleadoOpcional(
    IN p_Apellido VARCHAR(20)
)
BEGIN
...
-- 7.Salario, oficio, apellido y departamento por coincidencia
CREATE PROCEDURE BuscarPorApellidoParcial(
...
BEGIN
    ...
    JOIN Departamentos d
        ON e.IdDepartamento = d.IdDepartamento
    WHERE e.Apellido LIKE CONCAT('%', p_Valor, '%');
END //
DELIMITER ;